...
- **RDS Basics** — Amazon Relational Database Service — database engines (MySQL, PostgreSQL, MariaDB, Aurora), creating and connecting to an RDS instance, automated backups and manual snapshots, Multi-AZ high availability with automatic failover, Read Replicas for read scaling, security hardening (VPC, subnet groups, security groups, encryption at rest and in transit), Parameter Groups for engine tuning, monitoring with CloudWatch metrics and Performance Insights, and Aurora RDS (distributed storage architecture, cluster endpoints, Serverless v2, Global Database, and Backtrack).
